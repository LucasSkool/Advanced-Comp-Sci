...
public class Student {
    ...
    public Student(String name, String password, String email) {
        ...
        requireEmail(email);
        if (name.contains("@")) {
            throw new IllegalArgumentException("Name cannot include '@'."); // DONE
        }
        if (nameExists(name)) {
            throw new IllegalStateException("Display name taken."); // DONE
        }
        addName(name);
        this.name = name;
    ...
    public String resetPassword() {
        if (name == null || name.trim().isEmpty()) {
            throw new IllegalStateException("Student name missing."); //DONE
        }
    ...
    private static void requireNotBlank(String s, String field) {
        if (s == null) {
            throw new IllegalArgumentException(field + " is null."); // DONE
        }
        if (s.trim().isEmpty()) {
            throw new IllegalArgumentException(field + " is blank."); // DONE
        }
    }
    ...
    private static void requireEmail(String email) {
        ...
        if (!email.contains("@") || !email.contains(".")) {
            throw new IllegalArgumentException("Invalid email."); //DONE
        }
    }
}
